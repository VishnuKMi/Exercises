 ...
 * @notice  This contract is VERY loosely based on DAI on the MakerDAO (DAI) system.
 *
...
contract DSCEngine is ReentrancyGuard {
    ...
    /**
     * @notice  Violates CEI pattern but this can be an exception considering gas efficiency.
     * @notice  You can partially liquidate a user.
     * @notice  You will get a liquidation bonus for taking ther users fund.
    ...
    // 2. Private & Internal View/Pure Functions
    ...
    function _calculateHealthFactor(uint256 totalDscMinted, uint256 collateralValueInUsd)
        ...
        returns (uint256)
    {
        if (totalDscMinted == 0) return type(uint256).max; // To mitigate the issue when collateral is deposited and no corresponding mint is done.
        uint256 collateralAdjustedForThreshold = (collateralValueInUsd * LIQUIDATION_THRESHOLD) / LIQUIDATION_PRECISION;
        return (collateralAdjustedForThreshold * PRECISION) / totalDscMinted;
    }

    // 3. Public & External View/Pure Functions
